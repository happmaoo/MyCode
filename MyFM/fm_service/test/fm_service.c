...
static float seek(int fd, int dir) {

    LOGI("SEEK start dir=%d", dir);

    /* 1. 开始搜索 */
    set_control(fd, V4L2_CID_PRIVATE_IRIS_SRCHON, 1, "SRCHON");

    struct v4l2_hw_freq_seek s;
    memset(&s, 0, sizeof(s));
    s.tuner = 0;
    s.type = V4L2_TUNER_RADIO;
    s.seek_upward = dir ? 1 : 0;
    s.wrap_around = 1;

    if (ioctl(fd, VIDIOC_S_HW_FREQ_SEEK, &s) < 0) {
        LOGI("HW SEEK failed: %s", strerror(errno));
        set_control(fd, V4L2_CID_PRIVATE_IRIS_SRCHON, 0, "SRCHON");
        return -1;
    }

    /* 2. 等待 SRCHON 清 0（等价 hci_ev_search_compl） */
    int searching = 1;
    struct v4l2_control ctrl = {
        .id = V4L2_CID_PRIVATE_IRIS_SRCHON
    };

    for (int i = 0; i < 60; i++) {   // ~3 秒
        usleep(50000);

        if (ioctl(fd, VIDIOC_G_CTRL, &ctrl) == 0) {
            searching = ctrl.value;
            if (searching == 0)
                break;
        }
    }
    ...
    /* 3. 恢复接收状态 */
    //set_control(fd, V4L2_CID_PRV_STATE, FM_RECV, "FM_RECV");
    ...
    float mhz = f.frequency / 16 / 1000.0f;
    LOGI("SEEK done: %.2f MHz", mhz);
