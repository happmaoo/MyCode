...
//=================== 这里有很多seek作用相同，都是优化升级的版本留着老版本备用 =================
// =========== seek22 =========
int seek22(int fd,int dir) {
// 1. 设置信号强度阈值（例如：20 dBμV）
    struct v4l2_control ctrl;

    // 2. 启动 SEEK 搜索（向上）
    ctrl.id = V4L2_CID_PRIVATE_IRIS_SRCHON;
    ctrl.value = dir; // 非零表示开启搜索（方向通常由驱动内部默认为 UP）

    LOGI("Starting SEEK search...\n");
    if (ioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
        perror("Failed to start search");
        close(fd);
        return EXIT_FAILURE;
    }

    // 3. 等待搜索完成（简单方式：sleep；更优方式应监听事件）
    sleep(2); // SEEK 通常很快，1~2 秒足够
    ...
    // V4L2 频率单位是 62.5 kHz（对于 FM）
    double fm_freq_mhz = freq.frequency /16 / 1000.0; // 转换为 MHz

    LOGI("Search completed. Current frequency: %.2f MHz\n", fm_freq_mhz);
    return freq.frequency / 16;
}
...
int seek(int fd,int dir) {
    ...
    // 5. 输出结果
    double final_mhz = curr_freq.frequency/16 / 1000.0;
    if (found) {
        LOGI("✅ Seek succeeded! Tuned to:  %.2f MHz\n", final_mhz);
    } else {
        LOGI("⚠️ Search timeout or no station found. Still at:  %.2f MHz\n", final_mhz);
    }
    return curr_freq.frequency/16;
}


// =========== seek =========
int seek111(int fd,int dir) {
  int ret = 0;
