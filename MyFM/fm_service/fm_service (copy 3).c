...
// === 辅助函数：获取 xxxx 值 ===
int get_xxxxxx(int fd) {
    struct v4l2_control ctrl = { .id = V4L2_CID_PRIVATE_IRIS_ANTENNA };

    if (ioctl(fd, VIDIOC_G_CTRL, &ctrl) < 0) {
        printf("Error getting ANTENNA: %s\n", strerror(errno));
        LOGI("Error getting ANTENNA: %s\n", strerror(errno));
        return -1; // 返回错误代码
    }
    return ctrl.value;
}
...
// ----------------------------- 客户端发来的参数 -----------------------------------------------
    ...
    // 9. 获取xx: xxx
    else if (strncmp(cmd, "ANTENNA", 7) == 0) {
        //get_antenna(radio_fd);
    }
